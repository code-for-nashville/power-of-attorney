...
/*
    Returns a two column entry for an address object and label
*/
function addressEntryDefinition(label, address) {
    return {
        columns: [
        {
            text: label,
...
/*
  Create a page of instruction, followed by the PDF form, for each child's name

  original-form.pdf is a copy of the form this document definition is based on.  It's a useful reference.

  pdf-document.test.js uses this definition to render an example PDF (pdf-document-test.pdf). Running `yarn test` will regenerate pdf-document-test.pdf each time you make a change.  Keep pdf-document-test.pdf open to make sure the generated document looks the way it should.
*/
let createDocDefinition = (inputInfo, childName) => {
  const checked = (status) => status == inputInfo.parentalStatus ? ' X ' : '___';
  const statusOne = checked(PARENTAL_STATUS_LIVING);
  const statusTwo = checked(PARENTAL_STATUS_DECEASED);
  const statusThree = checked(PARENTAL_STATUS_LEGAL_CUSTODY_SIGNED);
  const statusFour = checked(PARENTAL_STATUS_LEGAL_CUSTODY_SENT);

  let multipleFrom = inputInfo.childrenNames.map((child) => {
    return([
      {
        text:'TODO: Instructions to sign and notarize each form.',
        style: 'header',
        pageBreak: 'after'
      },
      {
...
`,
      ...
      addressEntryDefinition(
          ...
          inputInfo[MOTHER_ADDRESS]
      ),
      addressEntryDefinition(
          '3. Father/Legal Guardian’s Name & Address:',
          inputInfo[FATHER_ADDRESS]
      ),
      addressEntryDefinition(
          '4. Caregiver’s Name & Address:',
          inputInfo[CAREGIVER_ADDRESS]
      ),
      {
        columns: [
        {
        ...
        {
            // Since the user is choosing in the form, can we only print the selected option?
            text: [
                `(${statusOne}) Both parents are living, have legal custody of the minor child and have signed this document\n`,
                { text: 'OR\n', bold: true },
                `(${statusTwo}) One parent is deceased;\n`,
                { text: 'OR\n', bold: true },
                `(${statusThree}) One parent has legal custody of the minor child and both parents have signed this document and consent to the appointment of the caregiver;\n`,
                { text: 'OR\n', bold: true },
                `(${statusFour}) One parent has legal custody of the minor child, and has sent by Certified Mail, Return Receipt requested, to the other parent at last known address, a copy of this document and a notice of the provisions in § 34-6-305; or the non-custodial parent has not consented to the appointment and consent cannot be obtained because ______________________________.\n\n`
            ],
            width: '*',
            margin: [5, 0, 0, 0]
        }
  ...
  return {
      ...
      styles: {
          ...
          addressColumn: {
              // PDFMake strips leading and trailing whitespace, but can still
              // use a margin to put space between our columns
              margin: [3, 0, 0, 0]
          }
      }
  };
